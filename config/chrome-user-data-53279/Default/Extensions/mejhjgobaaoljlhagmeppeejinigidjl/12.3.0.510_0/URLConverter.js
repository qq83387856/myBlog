function ConvertPunyCode( url )
{
	...
	var host = a.hostname;
	var punysign = "xn--";
	var res = host.indexOf( punysign );
	if( res != -1 )
	{
		output = a.protocol;
		output += "//";
		var spresult = host.split(".");
		for (var i = 0; i < spresult.length; i++)
		{
			res = spresult[i].indexOf( punysign );
			if( res == 0 )
			{
				tmp = spresult[i].substring(punysign.length);
				output += Punycode.decode(tmp);
			}
			else
			{
				output += spresult[i];
...
function init_JIS_TO_UTF8_TABLE() {
  if (JIS_TO_UTF8_TABLE === null) {
    ...
    var keys = Object.keys(UTF8_TO_JIS_TABLE);
    var i = 0;
    var len = keys.length;
    var key, value;
    ...
    keys = Object.keys(UTF8_TO_JISX0212_TABLE);
    len = keys.length;
