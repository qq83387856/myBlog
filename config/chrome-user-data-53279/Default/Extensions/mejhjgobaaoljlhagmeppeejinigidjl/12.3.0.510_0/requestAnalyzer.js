...
SkyFrame.WebRequest.RequestAnalyzer.analyzeCommon = function(request)
{
	try {
		...
		if(false == logchrome.analyzeRequestTarget(url, method, header)) {
			return;	// 次の解析処理へ
		}
		...
		if (null == request.dataId) {
			if (null != request.body.requestBody) {
				if (null != request.body.requestBody.raw) { 
					// Dropboxの場合はURLのみで判定できるため、ダミーデータを入れることで
					// データそのものを解析処理に送ることを防ぐ
					// 次バージョンにて改善
					if (0 < url.indexOf(".dropbox.com/chunked_upload?") ||
					    0 < url.indexOf(".dropbox.com/file_collector_chunked_upload?")
					) {
						try {
							var postDataUtf8Array = new TextEncoder().encode("<Dummy>");
		...
		var result = logchrome.analyzeRequest(url, method, header, request.dataId);
		if (null == result) {
			// 異常な場合は即時禁止
			return {cancel: true};
		}
...
SkyFrame.WebRequest.RequestAnalyzer.analyzeOffice = function(request)
{
	try {
		...
		if ("POST" == method) {
			...
			logchrome.isofficeposturl( url, tabId);
			return;		// 次の解析処理へ
		} else {
			return;		// 次の解析処理へ
		}
		
		if(false == logchrome.isofficeposturl( url, tabId)) {
			return;		// 次の解析処理へ
		}
		...
		if (null != body.raw) {
			if(0 < body.raw.length) {
				if( null != body.raw[0].bytes ) {
					if (null == request.dataId) {
						//request.dataId = logchrome.sendPostData([body.raw[0]]);
...
SkyFrame.WebRequest.RequestAnalyzer.analyzeOutlook = function(request)
{
	try {
		var url		= request.header.url;	// 未変換のURLを使用する
		if (false == logchrome.isoutlookwebuploadurl(url)) {
			return;
		}
	...
	try {
		...
		if (("POST" == method) || ("GET" == method)) {
			...
			try {
				...
				// ★ Javascript側で判定しないと無駄にC++側に処理がいってしまう問題に対する暫定対処
				// ★ 要改善
				var urlLower = request.url.toLowerCase();
				if (-1 == urlLower.indexOf("graph.microsoft.com")) {
					return;	// PUTの解析対象URLが見つからない場合は次の解析処理へ
				}
			} catch(ex) {
				return;	// エラーの場合は見つからないと同義として次の解析処理へ
			}
		}
		...
		chrome.tabs.get(tabId, function callback(tab){
			try {
				logchrome.onedriveaccess( url, tab.title, tabId );
			}
			catch(e){}
		} );

		var prohibitAccess 	= logchrome.isonedriveaccessforbidden( url, tabId);
		if(prohibitAccess){
			// 即時禁止
			return {cancel: true};
		}
