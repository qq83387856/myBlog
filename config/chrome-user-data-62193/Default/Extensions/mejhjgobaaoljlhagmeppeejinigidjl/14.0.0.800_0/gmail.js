...
//2018/01/16 shouji 現在は、outlook.js,outlookcom.jsは削除されています。
...
var CLogGMailAnalyser = function(doc) {
	...
	this.getItemToLogStringAttachmentNew = function (searchDoc) {
		...
		try {
			// Get attachment file from drive link
			//	<div class="gmail_chip gmail_drive_chip">
			//		<a aria-label="Firefox Setup 58.0.2.dat">
			//			<img>
			//			<span>
			//				Firefox Setup 58.0.2.dat
			//			</span>
			//		</a>
			//		<img>
			//	</div>
			$("div.gmail_chip.gmail_drive_chip > a", searchDoc).each( function() {
				var filename = $(this).attr("aria-label");
				if (0 < filename.length) {
					retValues.push(filename);
				}
			});
		} catch(ex2){}
