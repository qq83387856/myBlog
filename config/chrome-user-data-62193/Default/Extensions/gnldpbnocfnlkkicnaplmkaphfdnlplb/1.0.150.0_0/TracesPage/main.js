...
define('TracesPage',["require", "exports", "../Tracing/Tracing", "../Common/ExtensionStorage", "../Common/PersistentStorageProxy"], function (require, exports, Tracing, ExtensionStorage) {
    var tracingService = Tracing.TracingService;
    var moduleName = "TracesPage";
    /* Triggers the function call on click of show Traces. */
    $('document').ready(function () {
        showTraces();
    });
    function showTraces() {
        var storageService = ExtensionStorage.PersistentStorageFactory.getStorageService();
        // Each log file is in the format xt-console-traces-{DayCount}
        for (var i = 0; i <= 7; i++) {
            (function (i) {
                var fileName = tracingService.traceFileNamePrefix + i;
                var traceFileName = getTraceFileName(fileName);
                storageService.getBlob(traceFileName, function (data) {
                    var pathSplitter = traceFileName.split("/");
                    if (data) {
                        insertBlobInTable(pathSplitter[1], data);
                    }
                }, function (error) { }, false);
            })(i);
        }
    }
    function getTraceFileName(name) {
        var fileName = tracingService.tracingDirectoryName + "/" + name;
        return fileName;
    }
    function insertBlobInTable(fileName, blob) {
        var row = getTraceRow(fileName, blob);
        $(".traces-table").append(row);
    }
    function getTraceRow(fileName, blob) {
        var url = URL.createObjectURL(blob);
        var actionLink = '<a target="_blank" href="' + url + '" >' + fileName + '</a>';
        var column1 = '<td class = "trace-column-1 trace-content-column">' + actionLink + '</td>';
        var size = formatBytes(blob.size);
        var column2 = '<td class = "trace-column-2 trace-content-column">' + size + '</td>';
        var row = '<tr class="traces-row">' + column1 + column2 + '</tr>';
        return $(row);
    }
