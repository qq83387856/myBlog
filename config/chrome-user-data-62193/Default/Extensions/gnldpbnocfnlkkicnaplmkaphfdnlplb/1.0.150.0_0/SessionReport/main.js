...
// TODO: Enable tracing. Tracing is not getting loaded after optimization for this module only as it uses var instead of import, will fix it.
//var Tracing = require("../Tracing/Tracing");

//TODO: remove this global usage
var PersistentStorageProxy;
var quotaIncreaseAmount = 10 * 1024 * 1024; /*10MB*/
var moduleName = "PersistentStorageProxy";

(function (PersistentStorageProxy) {
    function writeToFileSystem(fileSystem, grantedBytes, fileName, blob, onSuccess, onFailure, appendContent) {
        //Tracing.TracingService.traceEnter("writeToFileSystem", moduleName, 1, "Writing to file ");
                ...
                fileEntry.createWriter(function (fileWriter) {
                    ...
                    fileWriter.onwrite = function (e) {
                        if (!truncated) {
                            ...
                            //Tracing.TracingService.traceLeave("writeToFileSystem", moduleName, 2, "Wrote to file " + fileEntry.toURL());
    ...
    function writeBlob(fileName, blob, onSuccess, onFailure, appendContent) {
        var fileSystem = window.requestFileSystem || window.webkitRequestFileSystem;
        if (!fileSystem) {
            //.TracingService.traceError(moduleName, 3, "FileSystem not supported by the browser");
        }
        navigator.webkitPersistentStorage.queryUsageAndQuota(function (usedBytes, grantedBytes) {
            if (grantedBytes - usedBytes < blob.size) {
                //Tracing.TracingService.traceWarning(moduleName, 4, "Quota limit reached, increasing the file system size");
                window.requestFileSystem = window.requestFileSystem || window.webkitRequestFileSystem;
                navigator.webkitPersistentStorage.requestQuota(grantedBytes + quotaIncreaseAmount, function(newGrantedBytes) {
                    //Tracing.TracingService.traceInfo(moduleName, 5, "Size of the file system increased to " + newGrantedBytes);
    ...
    function getBlob(fileName, onSuccess, onFailure) {
        var fileSystem = window.requestFileSystem || window.webkitRequestFileSystem;
        if (!fileSystem) {
            //Tracing.TracingService.traceError(moduleName, 6, "FileSystem not supported by the browser");
        }
        //Tracing.TracingService.traceEnter("getBlob", moduleName, 7, "Reading blob ");
        navigator.webkitPersistentStorage.queryUsageAndQuota(function(usedBytes, grantedBytes) {
                fileSystem(window.PERSISTENT, grantedBytes, function(fs) {
                    fs.root.getFile(fileName, { }, function(fileEntry) {
                        fileEntry.file(function (file) {
                            //Tracing.TracingService.traceLeave("getBlob", moduleName, 8, "getBlob succeeded for " + fileName);
                            onSuccess(file);
                        }, onFailure);
                    }, onFailure);
                }, onFailure);
    ...
    function removeBlob(fileName, onSuccess, onFailure) {
        var fileSystem = window.requestFileSystem || window.webkitRequestFileSystem;
        if (!fileSystem) {
            //Tracing.TracingService.traceError(moduleName, 9, "FileSystem not supported by the browser");
        }
        navigator.webkitPersistentStorage.queryUsageAndQuota(function(usedBytes, grantedBytes) {
            fileSystem(window.PERSISTENT, grantedBytes, function(fs) {
    ...
    function createDirectory(dirName, size, onSuccess, onFailure) {
        var fileSystem = window.requestFileSystem || window.webkitRequestFileSystem;
        fileSystem(window.PERSISTENT, size, function (fs) {
            fs.root.getDirectory(dirName, {create: true}, function(dirEntry) {
                onSuccess(dirEntry);
            }, onFailure);
        }, onFailure);
    }
    function removeDirectory(dirName, size, onSuccess, onFailure) {
        var fileSystem = window.requestFileSystem || window.webkitRequestFileSystem;
        fileSystem(window.PERSISTENT, size, function (fs) {
            fs.root.getDirectory(dirName, {create: true}, function (dirEntry) {
    ...
    PersistentStorageProxy.writeBlob = writeBlob;
    PersistentStorageProxy.getBlob = getBlob;
    PersistentStorageProxy.removeBlob = removeBlob;
    PersistentStorageProxy.createDirectory = createDirectory;
    PersistentStorageProxy.removeDirectory = removeDirectory;
})(PersistentStorageProxy || (PersistentStorageProxy = {}));

define("../Common/PersistentStorageProxy", function(){});

define('SessionReport',["require", "exports", "../Model/Model", "../Tracing/Tracing", "../Common/ExtensionStorage", "../Model/BrowserHelper", "../Messaging/ChromeMessaging", "../Messaging/Messaging", "../Model/ModelHelper", "../Common/Utility", "../Common/PersistentStorageProxy"], function (require, exports, Model, Tracing, ExtensionStorage, BrowserHelp, ChromeMessaging, Messaging, ModelHelper, Utility_1) {
    var BrowserHelper = BrowserHelp.BrowserHelper;
    var tracingService = Tracing.TracingService;
    var moduleName = "SessionReport";
    /* Triggers the function call on export of the session report. */
    $('document').ready(function () {
        fixToc();
        tracingService.traceVerbose(moduleName, 1, "Exported report dom ready, reading exported session object from blob");
        var session = ExtensionStorage.LocalStorageService.retrieve(ExtensionStorage.offlineReport);
        generateReport(JSON.stringify(session));
    });
    function fixToc() {
        var tocWitIds = document.getElementsByClassName('tocWitId');
        [].forEach.call(tocWitIds, function (tocAnchor) {
            var witId = tocAnchor.innerText;
            tocAnchor.setAttribute("href", "#" + witId);
        });
    }
    ...
    function generateReport(sessionString) {
        tracingService.traceVerbose(moduleName, 3, "Generating session report");
        var session = JSON.parse(sessionString);
        // Fill report stats
        $('#ReportStats').html(getReportStats(session));
        // Table of content div
        $('#TableOfContent').html(getTocDiv(session));
        // Workitems div
        $('#WorkItems').html(getWorkItemsDiv(session));
        // Report generator text
        $('#ReportGeneratorText').html(getReportGeneratorText());
        // Report footer text
        $('#ReportFooter').html(getReportFooterInfoText());
        // Session Attachments div
        $('#SessionAttachments').html(getSessionAttachmentsDiv(session));
        transformImages();
    }
    function getReportGeneratorText() {
        return 'This report was generated using <a href="http://aka.ms/xtmarketplace">Test & Feedback extension</a> for <a href="https://www.azure.com/services/devops">' + Utility_1.BrandingChangesHelper.getHostedBrandingName() + '</a> / Team Foundation Server in standalone mode (with limited functionality). Connect to' + ' ' + Utility_1.BrandingChangesHelper.getHostedBrandingShortName() + ' for the full featured version, learn more - <a href="http://aka.ms/xtlearn">http://aka.ms/xtlearn</a>.';
    }
    function getReportFooterInfoText() {
        return 'Want to track bugs in a more seamless manner? Connect to <a href="https://www.azure.com/services/devops">' + Utility_1.BrandingChangesHelper.getHostedBrandingName(true) + '</a> for free.';
    }
    function getReportStats(session) {
        var createdTime = session.createdTime.toString();
        var timeZone = new Date().toString().match(/\((.*)\)/)[1];
        var sessionCreatedDatetime = getFormattedDate(new Date(createdTime));
        ;
        var sessionExportDatetime = getFormattedDate(new Date());
        var reportStats = sessionCreatedDatetime + '<br>'
            + sessionExportDatetime + '<br>'
            + session.workItems.length + '<br>'
            + timeZone;
        return reportStats;
    }
    /*
        we are already sanitizing all the inputs over here
        1. getWorkItemId & getWorkItemTitle functions return encoded html
        2. getFormattedDate returns a date
        3. workItem.description is already sanitized. Taken care while storing it in local storage from popup. Refer:
        Extension\Popup2\BugForm\BugForm2.Controller.ts(getSelectedBugDataInWorkItem() ln 537)
    */
    function getWorkItemsDiv(session) {
        var witDiv = '</br></br>';
        session.workItems.forEach(function (workItem) {
            witDiv += '<div class="GreyedTileDiv"><div class="WorkItemId" id="' + getWorkItemId(workItem) + '"><p class="inlineBlock">' + getWorkItemId(workItem) + '</p></div>'
                + '<div class="WorkItemDate" ><p class="inlineBlock">' + getFormattedDate(new Date(workItem.timeStamp.toString())) + '</p></div><br/>'
                + '<div class="WorkItemTitle">' + getWorkItemTitle(workItem) + '</div></div>'
                + '<div class="line"></div><div class="WorkItemDescription"><p class="inlineBlock">' + workItem.description + '</p></div>';
        });
        return witDiv;
    }
    /*
        we are already sanitizing all the inputs over here
        1. getAttachmentName function returns encoded html
        2. getFormattedDate returns a date
        3. attachment.localUrl is a url and is html safe
        4. getAttachmentTextValue sanitized attachment.textValue
    */
    function getSessionAttachmentsDiv(session) {
        var sessionAttachmentsDiv = '</br></br><div class="TocTitleDiv"><div class="TocHeader"><p class="inlineBlock">Session attachments</p></div></div><div class="line"> </div>';
        session.attachments.forEach(function (attachment) {
            sessionAttachmentsDiv += '<br/><div class="WorkItemId" ><p class="inlineBlock">' + getAttachmentName(attachment) + '</p></div>'
                + '<div class="WorkItemDate" ><p class="inlineBlock">' + getFormattedDate(new Date(attachment.timeStamp.toString())) + '</p></div><br/>'
                + '<div class="WorkItemDescription">';
            if (attachment.type === Model.AttachmentType.Screenshot) {
                sessionAttachmentsDiv += '<img src="' + attachment.localUrl + '"/></div></br></br>';
            }
            else if (attachment.type === Model.AttachmentType.Note) {
                sessionAttachmentsDiv += getAttachmentTextValue(attachment) + '</div></br></br>';
            }
        });
        return sessionAttachmentsDiv;
    }
    /*
        we are already sanitizing all the inputs over here
        1. getWorkItemId & getWorkItemTitle functions return encoded html
        2. getFormattedDate returns a date
    */
    function getTocDiv(session) {
        var tocDiv = '';
        session.workItems.forEach(function (workItem) {
            tocDiv += '<div class="ellipses"><div class="line"> </div>'
                + '<div class="TocWorkItemId"><a class="tocWitId" href="#' + getWorkItemId(workItem) + '">' + getWorkItemId(workItem) + '</a></div>'
                + '<div class="TocWorkItemDate">' + getFormattedDate(new Date(workItem.timeStamp.toString())) + '</div>'
                + '<div class="TocWorkItemTitle">' + getWorkItemTitle(workItem) + '</div></div>';
        });
        return tocDiv;
    }
    function getDownloadReportUrl() {
        var body = '<body>' + $("body")[0].innerHTML + '</body>';
        var style = '<style>' + $("style")[0].innerHTML + '</style>';
        var reportHtml = '<html>' + style + body + '</html>';
        var reportBlob = new Blob([reportHtml], { type: "text/html" });
        return (window.URL.createObjectURL(reportBlob));
    }
    function getWorkItemTitle(workItem) {
        var textArea = document.createElement('textarea');
        textArea.textContent = workItem.title;
        return textArea.innerHTML;
    }
    function getWorkItemId(workItem) {
        var textArea = document.createElement('textarea');
        textArea.textContent = "Bug " + workItem.id;
        return textArea.innerHTML;
    }
    function getAttachmentName(attachment) {
        var textArea = document.createElement('textarea');
        textArea.textContent = attachment.name;
        return textArea.innerHTML;
    }
    function getAttachmentTextValue(attachment) {
        var textArea = document.createElement('textarea');
        textArea.textContent = attachment.textValue;
        return textArea.innerHTML;
    }
    ...
    function transformImages() {
        tracingService.traceVerbose(moduleName, 4, "Transforming images from local url to data url");
        var imagesToBeConverted = 0;
        // Count number of images to be converted.
        $("img").each(function () {
        ...
        // Transform images to inline images.
        if (BrowserHelper.isEdge()) {
            $("img").each(function () {
                var currentImage = this;
                ChromeMessaging.MessagingFactory.getBroadcastingService().broadcast(Messaging.NotificationType.LoadScreenshotContentFromBlob, { url: currentImage.src }, function (dataUrl) {
            ...
            $("img").each(function () {
                var currentImage = this;
                if (isValidImageUrl(currentImage)) {
                    ModelHelper.convertToDataUrl(currentImage.src, function (url) {
                        currentImage.src = url;
                        imagesToBeConverted--;
                        // If all images have been converted then generate download session report link.
                        if (imagesToBeConverted === 0 && BrowserHelper.isChrome()) {
                            var reportHref = getDownloadReportUrl();
                            var link = document.createElement("a");
                            link.setAttribute("href", reportHref);
                            link.setAttribute("download", "XTSessionReport.html");
    ...
    function isValidImageUrl(currentImage) {
        return currentImage.src.startsWith("filesystem:" + BrowserHelper.chromeExtensionHost)
            || currentImage.src.startsWith("blob:" + BrowserHelper.firefoxExtensionHost)
            || currentImage.src.startsWith("blob:")
            || currentImage.src.startsWith(BrowserHelper.chromeExtensionHost)
            || currentImage.src.startsWith(BrowserHelper.firefoxExtensionHost);
    }
});
